...
def Consultar_Title(bd):
    ...
    for publicação in bd:
        if publicação.get('title') and publicação['title'].lower().strip(' .') == busca_title.lower():
...
def Atualizar_Pub(bd): 
    ...
    for publicação in bd:
        if publicação.get('title') and busca_title == publicação['title'].lower().strip(' .'):
...
def Listar_Pub_PC(bd):
    ...
    palavra_chave = input('Introduza a palavra-chave que deseja procurar: ').lower()
